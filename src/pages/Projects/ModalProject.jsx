...
import { useCreateKanbanMutation } from "../../features/kanban/KanbanApiSlice";
...
const ModalProject = ({ open, setOpen }) => {
  const [createKanban, isLoading] = useCreateKanbanMutation();
  ...
  const handleClick = async () => {
    try {
      ...
      await createKanban(projectInfo);
    } catch (error) {
  ...
  const handleChange = (e) => {
    if (e.target.name == "isPrivate") {
      setProjectInfo({ ...projectInfo, [e.target.name]: e.target.checked });
    } else setProjectInfo({ ...projectInfo, [e.target.name]: e.target.value });
  };
